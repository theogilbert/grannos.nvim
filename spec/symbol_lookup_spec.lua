...
--- Open a connected SQL buffer holding `text`, cursor on the first `needle`.
--- @param text   string
--- @param needle string
--- @return integer  bufnr
local function connected_sql_buf(text, needle)
  local key = connections.conn_key("local", "postgres", "", "test")
...
describe("describe_symbol_at_cursor progress", function()
  ...
  it("marks the symbol's line while the lookup is in flight", function()
    local buf = connected_sql_buf("SELECT * FROM users;", "users")
